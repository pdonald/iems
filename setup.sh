...
sudo apt-get -qq install libboost-all-dev libgoogle-perftools-dev libsparsehash-dev libeigen3-dev gtest-dev google-mock
...
cd mosesdecoder
./bjam -a -j`nproc` --with-mm --with-cmph=$WORK/cmph-2.0/build
